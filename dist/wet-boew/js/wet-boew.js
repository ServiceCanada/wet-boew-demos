...
var $document = wb.doc,
	...
	toggleClassAct = function( event, data ) {
		var $elm = $( data.source || event.target );
		if ( !data.class ) {
			return;
		}
		$elm.toggleClass( data.class );
	},
	...
	importInDataset = function( event, data ) {
		...
		function recursiveFieldset( inField ) {
			...
			if ( inField.type === "fieldset" && inField.dataset.path && inField.dataset.templateObject ) {
				...
				inField.myNewValue = compiledValue;
			}
		...
		for( var i = 0; i < arrInputToProcess.length; i++ ) {
			...
			default:
				...
				default:
					if ( inField.type === "select-one" && data.useTextValue ) {
						inputValue = inField.options[ inField.selectedIndex ].text;
					} else {
						inputValue = inField.value;
					}
				}

				if ( inField.myNewValue  ) {
					inputValue = inField.myNewValue;
				}
				...
				if ( inField.type === "fieldset" && ( inField.value || inField.myNewValue ) ) {
					delete inField.value;
					delete inField.myNewValue;
				}
			}
...
var componentName = "wb-data-json",
	...
	jsonFetched = function( event ) {
		...
		if ( showEmpty || typeOfContent !== "undefined" ) {
			...
			// Stringify content
			if ( jsonType && typeof content === "object" && content !== null ) {
			...
			if ( !jsonType ) {
				...
				$elm.html( stringContent );
			} else if ( jsonType === "replacewith" ) {
				$elm.replaceWith( stringContent );
			} else if ( jsonType === "addclass" ) {
				$elm.addClass( stringContent );
			} else if ( jsonType === "removeclass" ) {
				$elm.removeClass( stringContent );
			} else if ( jsonType === "prop" && attrname && allowPropNames.test( attrname ) ) {
				$elm.prop( attrname, stringContent );
			} else if ( jsonType === "attr" && attrname && allowAttrNames.test( attrname ) ) {
				$elm.attr( attrname, stringContent );
			} else if ( jsonType === "select" ) {

				var selector = "",
					optionsSelected;

				if ( !Array.isArray( stringContent ) ) {
					stringContent = [ stringContent ];
				}
				stringContent.forEach( function( itm ) {
			...
			},
			{
				...
				fn: function( obj, key, tree ) {
					if ( Array.isArray( this.value ) ) {
						applyPatch( tree, "add", this.path, $.extend( true, [], this.value ) );
					} else {
						applyPatch( tree, "add", this.path, $.extend( true, {}, this.value ) );
					}
				}
			...
			},
			{
				...
				fn: function( obj, key, tree ) {
					...
					val =  String(val)
						...
						.replace(/[\u0300-\u036f]/g, '') // remove all the accents, which happens to be all in the \u03xx UNICODE block.
					...
					if ( !this.set ) {
						applyPatch( tree, "add", this.path, val );
					} else {
						applyPatch( tree, "add", this.set, val );
					}
				}
			},
			{
				...
				fn: function( obj, key, tree ) {
					...
					applyPatch( tree, "add", this.path, newVal );
				}
			},
			{
				name: "wb-swap",
		...
		opsArray: [
			...
			},
			{
				...
				fn: function( arr ) {
					arr.forEach( ( item, i ) => {
						if ( this.set ) {
							jsonpatch.apply( arr, [
								{ op: "wb-slug", path: "/" + i + this.path, set: "/" + i + this.set, mainTree: this.mainTree }
							] );
						} else {
							jsonpatch.apply( arr, [
								{ op: "wb-slug", path: "/" + i + this.path, mainTree: this.mainTree }
							] );
						}
					} );
				}
			},
			{
				...
				fn: function( arr ) {
					arr.forEach( ( item, i ) => {
						jsonpatch.apply( arr, [
							{ op: "wb-stringify", path: "/" + i + this.path, separator: this.separator, mainTree: this.mainTree }
						] );
					} );
				}
			},
			...
			},
			{
				...
				fn: function( arr ) {
					arr.forEach( ( item, i ) => {
						jsonpatch.apply( arr, [
							{ op: "wb-add", path: "/" + i + this.path, value: this.value }
						] );
					} );
				}
			},
